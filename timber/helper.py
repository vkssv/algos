...
if __name__ == "__main__":
	...
	usage = "usage: %prog [options] -t training_directory -f file"
	...
	try:
		...
		for path, subdirs, docs in os.walk(options.train_dir):
			for d in docs:
				...
				headers_parser(cut_header_from_body(sample_path)[0])

				logger.debug ("============== parser from STL email ====================\n")
				for k in msg.keys():
					logger.debug("HEADER: " +k+' ==> '+str(msg.get(k)))
				logger.debug ("EPILOGUE:" + str (msg.epilogue))
				logger.debug ("PREAMBLE:" + str (msg.preamble))
