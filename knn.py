...
def create_doc_vector (doc_path):
	...
	#                 (size|SUS_subj|SUS_Rcpt|From|To|Cc|Bcc|SUS_url|Auth-Res|DMARC|DKIM|SPF|Sender|X.*|Err|X-Mailer|User-Agent|Content-Type|Mime-Version|Reply-To|Content-Language|body_score
...
if __name__ == "__main__":
	...
	parser.add_option("-t", action="store", type="string", dest="train_dir", metavar="[REQUIRED]", help="path to dir with spam/ham collections")
	parser.add_option("-f", action="store", type="string", dest="new_doc", metavar="[REQUIRED]", help="path to checking email" )
	parser.add_option("-k", type="int", dest="k", default=3, metavar=" ", help="count of nearest neighbors, default k=3")
	parser.add_option("-v", action="store_true", dest="verbose", default=False, metavar=" ", help="be verbose")
